...
import { NODE_TYPE_COMPONENT, NODE_TYPE_CONSUMER, CORR_ID_SCOPE_INTERACTION, CORR_ID_SCOPE_CAUSED_BY } from './constants';
...
class APMSpan {
    ...
    _initChildOf(primaryReference, otherReferences) {
        ...
        let nodeType = NODE_TYPE_COMPONENT;
    ...
    _initFollowsFromOrJoin(primaryReference, otherReferences) {
        ...
        const initialConsumer = new Node(null, NODE_TYPE_CONSUMER, corrIds);
        ...
        initialConsumer.getNodes().push(new Node(this, NODE_TYPE_COMPONENT));
    ...
    static remainingReferencesCorrIds(references) {
        ...
        for (let i = 0; i < references.length; i += 1) {
            let scope = CORR_ID_SCOPE_CAUSED_BY;
    ...
    static primaryCorrelationId(reference) {
        function correlationIdScope(referenceType) {
            let corrIdScope = CORR_ID_SCOPE_INTERACTION;
            switch (referenceType) {
            case Constants.REFERENCE_CHILD_OF:
                corrIdScope = CORR_ID_SCOPE_INTERACTION;
                break;
            case Constants.REFERENCE_FOLLOWS_FROM:
                corrIdScope = CORR_ID_SCOPE_CAUSED_BY;
                break;
            default:
