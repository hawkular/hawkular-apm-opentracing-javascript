...
const CARRIER_PREFIX = 'hawkular-apm-';
const CARRIER_SPAN_ID = `${CARRIER_PREFIX}spanid`;
const CARRIER_TRACE_ID = `${CARRIER_PREFIX}traceid`;
const CARRIER_SAMPLED = `${CARRIER_PREFIX}sampled`;

class APMTracer {
    constructor(prop = {}) {
        this._recorder = prop.recorder || new ConsoleRecorder();
        this._sampler = prop.sampler || new AlwaysSampledSampler();
    }
    ...
    inject(spanContext, format, carrier) {
        ...
        if (format === Constants.FORMAT_HTTP_HEADERS ||
            format === Constants.FORMAT_TEXT_MAP) {
    ...
    static _injectHttpAndTextMap(spanContext, carrier) {
        ...
        carrier[CARRIER_SPAN_ID] = spanContext.getId();
    }
    ...
    extract(format, carrier) {
        if (format === Constants.FORMAT_HTTP_HEADERS ||
            format === Constants.FORMAT_TEXT_MAP) {
            return APMTracer._extractHttpAndTextMap(carrier);
        }

        console.log('Unsupported format');
        return new APMSpanContext();
    }
    ...
    static _extractHttpAndTextMap(carrier) {
        ...
        Object.keys(carrier).forEach((objectKey) => {
            ...
            case CARRIER_SPAN_ID:
                spanId = carrier[objectKey];
                break;
            case CARRIER_TRACE_ID:
                traceId = carrier[objectKey];
                break;
            case CARRIER_SAMPLED:
                break;
            // ignoring
            default:
                // error, but ignoring
        ...
        if (spanId && traceId) {
            return new APMSpanContext(spanId, traceId);
        }
