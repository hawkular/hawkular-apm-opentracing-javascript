...
class ConsoleRecorder extends NoOpRecorder {
    record(span) {
        console.log(JSON.stringify({
            ...
            startTime: span._startMillis,
            duration: span._endMillis - span._startMillis,
            logs: span.getLogs(),
            tags: span.getTags(),
        }));
    }
...
class HttpRecorder {
    constructor(url, username, password, debug) {
        ...
        if (debug) {
            this._consoleRecorder = new ConsoleRecorder();
        }
    }
    ...
    record(span) {
        ...
        if (this._consoleRecorder) {
            this._consoleRecorder.record(span);
        }
    }
}
